...
| **Navigation** | Le bloc est en iframe → liens inter-pages en `<a target="_top">` pour la NAVIGATION (barre d'onglets, Raccourcis : on veut y rester). Depuis le 2026-09-15, tout lien qui QUITTE l'accueil — « Ouvrir la fiche complète » de la pop-up, « Détail » d'une notification, les pieds « Ouvrir … » des widgets et du SAV — passe par **`NOUVEL_ONGLET`** (`_blank` + `noopener noreferrer`) : l'accueil reste ouvert, avec ses filtres et son défilement. Exige une URL **absolue** (`pageUrl` la fabrique sur `topOrigin()`). |
